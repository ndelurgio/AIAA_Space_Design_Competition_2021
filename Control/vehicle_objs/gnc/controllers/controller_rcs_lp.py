...
class Controller():
    ...
    def updateCtrlCmd(self,estimator,guidance):
        ...
        posErrorY = posError[1]
        #print(guidance.state_cmd.x)
        if abs(posErrorY) > 5.0:
            #self.ctrl_cmd["main_engine"] = [True,1.0]
            self.ctrl_cmd["rcs_pz_px_my"] = [True,1.0]
            self.ctrl_cmd["rcs_pz_mx_my"] = [True,1.0]
            self.ctrl_cmd["rcs_mz_px_my"] = [True,1.0]
            self.ctrl_cmd["rcs_mz_mx_my"] = [True,1.0]
        else:
            self.ctrl_cmd["rcs_pz_px_my"] = [False,1.0]
            self.ctrl_cmd["rcs_pz_mx_my"] = [False,1.0]
